/**
 * Trainer Clients Screen View — Dedicated Screen View for Gym Trainers on Tab 4
 * Replaces Fasting Timer with Athlete Client CRM, PAR-Q+ Injury Shield & 12-Pack Milestone Tracker.
 */
...
export function TrainerClientsScreenView() {
  const { clients, appointments, punchClientSession, renewClientPackage, addClient } = useTrainerStore();
  ...
  const handleConfirmRenewal = async () => {
    ...
    if (updated) {
      ...
      Alert.alert(
        ...
        `Successfully recorded ৳${priceNum.toLocaleString()} via ${renewalMethod}. Send official WhatsApp money receipt?`,
        [
          {
            text: 'Send Receipt',
            onPress: () => {
  ...
  return (
    <AppScreen style={{ backgroundColor: TC.screenBg }}>
      <AppScreenHeader
        title="ATHLETE ROSTER"
        subtitle="Client CRM & Safety Vault"
        containerStyle={{ backgroundColor: TC.screenBg }}
        titleStyle={{ color: TC.textPrimary }}
        subtitleStyle={{ color: TC.textSecondary }}
        rightAction={
          <TouchableOpacity
            activeOpacity={0.8}
            onPress={() => {
            ...
            <Text style={styles.enrollBtnText}>+ Client</Text>
          </TouchableOpacity>
        }
      />

      <ScrollView
        style={{ backgroundColor: TC.screenBg }}
        showsVerticalScrollIndicator={false}
        contentContainerStyle={styles.scrollContainer}>
        {/* COACH STUDIO TOOLBAR */}
        <View style={styles.coachOpsBar}>
          <TouchableOpacity
            activeOpacity={0.8}
            onPress={() => {
              void Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light).catch(() => {});
            ...
            style={styles.coachOpBtn}>
            <MaterialIcons name="payments" size={16} color={TC.forestDark} />
            <Text style={styles.coachOpBtnText}>Manage PT Packages & Rates</Text>
          </TouchableOpacity>
        </View>
        ...
        <View style={styles.statsBento}>
          ...
          <View style={styles.bentoItem}>
            <Text style={[styles.bentoVal, { color: TC.forestDark }]}>{appointments.length}</Text>
            <Text style={styles.bentoLbl}>Today's PT</Text>
          </View>
        </View>
        ...
        <View style={styles.clientsSection}>
          <Text style={styles.sectionHeader}>ATHLETE DIRECTORY ({filteredClients.length})</Text>
        ...
        <Modal
          ...
          <SafeAreaView style={styles.modalSafeArea} edges={['top', 'bottom']}>
            <View style={styles.modalHeaderRow}>
              ...
              <View style={{ flex: 1 }}>
                <Text style={styles.modalHeaderTitle}>{activeDossierClient.name.toUpperCase()}</Text>
                <Text style={styles.modalHeaderSubtitle}>Athlete Clinical Dossier & PAR-Q+ Vault</Text>
              </View>
              <TouchableOpacity
                activeOpacity={0.7}
                onPress={() => {
...
const styles = StyleSheet.create({
  ...
  // DOSSIER SHARE ACTIONS & PAYMENT LEDGER
  dossierFastShareRow: {
    flexDirection: 'row',
    gap: 8,
    marginTop: 8,
